...
-- Total number of rows in the table 'movie' is 7997
SELECT 
	COUNT(*) AS Total_Num_Of_Rows
FROM
	movie;

-- Total number of rows in the table 'genre' is 14662
SELECT 
	COUNT(*) AS Total_Num_Of_Rows
FROM
	genre;
...
-- Total number of rows in the table 'ratings' is 7997
SELECT 
	COUNT(*) AS Total_Num_Of_Rows
FROM
	ratings;
    
-- Total number of rows in the table 'role_mapping' is 15615
SELECT 
	COUNT(*) AS Total_Num_Of_Rows
FROM
	role_mapping;
    
-- Total number of rows in the table 'director_mapping' is 3867
SELECT 
	COUNT(*) AS Total_Num_Of_Rows
FROM
	director_mapping;
    
-- Total number of rows in the table 'names' is 25735
SELECT 
	COUNT(*) AS Total_Num_Of_Rows
FROM
	names;

-- Finding Row_Count for every Column in One Frame
SELECT
	(SELECT 
			COUNT(*)
		FROM 
	...
	(SELECT 
		COUNT(*)
		FROM 
			role_mapping) AS role_mappings_count,
     (SELECT 
		COUNT(*)
		FROM 
	 ...
	 (SELECT 
		COUNT(*)
		FROM 
			director_mapping) AS director_count;


-- Q2. Which columns in the movie table have null values?
-- Type your code below:

-- country,worldwide_gross_income,languages,production_company have null values
SELECT 
    (SELECT 
            COUNT(*)
        FROM
            movie
        WHERE
            id IS NULL) AS id_null,
    (SELECT 
            COUNT(*)
        FROM
            movie
        WHERE
            title IS NULL) AS title_null,
    (SELECT 
            COUNT(*)
        FROM
            movie
        WHERE
            year IS NULL) AS year_null,
    (SELECT 
            COUNT(*)
        FROM
            movie
        WHERE
            date_published IS NULL) AS date_published_null,
    (SELECT 
            COUNT(*)
        FROM
            movie
        WHERE
            duration IS NULL) AS duration_null,
    (SELECT 
            COUNT(*)
        FROM
            movie
        WHERE
            country IS NULL) AS country_null,
    (SELECT 
            COUNT(*)
        FROM
            movie
        WHERE
            worlwide_gross_income IS NULL) AS worlwide_gross_income_null,
    (SELECT 
            COUNT(*)
        FROM
            movie
        WHERE
            languages IS NULL) AS languages_null,
    (SELECT 
            COUNT(*)
        FROM
            movie
        WHERE
            production_company IS NULL) AS production_company_null;
...
SELECT 
	year AS Year , 
	COUNT(DISTINCT id) AS num_of_movies  
FROM 
	movie
WHERE
	(country LIKE '%USA%' 
...
SELECT 
    'German' AS movie_language, SUM(total_votes) AS Total_sum_of_votes
FROM 
	movie AS m
		INNER JOIN
	ratings AS r ON m.id = r.movie_id
...
UNION 
SELECT 
    'Italian' AS movie_language, SUM(total_votes) AS Total_sum_of_votes
FROM
    movie AS m
        INNER JOIN
    ratings AS r ON m.id = r.movie_id
...
-- find top 3 genres based on number of movies with average rating > 8
WITH genre_rank AS
(
SELECT 
	genre AS genre,
    RANK() OVER( ORDER BY COUNT(id) DESC) AS rank_by_genre
FROM 
	movie AS m
		INNER JOIN
    genre AS g ON m.id = g.movie_id
...
-- find top 3 directors using the top 3 genres
SELECT 
	name AS director_name,count(d.movie_id) AS movie_count
FROM
	...
	genre_rank
WHERE avg_rating > 8 AND g.genre in (genre_rank.genre)
GROUP BY name
ORDER BY movie_count DESC
LIMIT 3;
...
FROM 
	role_mapping AS r 
		INNER JOIN 
    names AS n ON n.id = r.name_id
		INNER JOIN 
	top_rated_movies AS t ON t.id = r.movie_id
GROUP BY name
ORDER BY movie_count DESC
LIMIT 2;
...
SELECT 
	name AS actress_name , SUM(total_votes) AS total_votes,
    COUNT(m.id) AS movie_count, 
...
WHERE category = 'actress' AND
		country ='India' AND languages ='Hindi'
GROUP BY name
...
ON g.movie_id = m.id
INNER JOIN top_3_genres AS t
USING(genre)
),
movie_rank_byincome AS
(
SELECT genre,
    title AS movie_name,
	year,
...
FROM movies_from_top3_genres
)
SELECT * 
FROM movie_rank_byincome
WHERE movie_rank <=5;
...
WITH multilingual_top_rated AS
(
...
SELECT name AS actress_name,
	SUM(total_votes) AS total_votes,
    count(id) AS movie_count,
    (SUM(avg_rating*total_votes)/sum(total_votes)) AS actress_average_rating,
    RANK() OVER(ORDER BY count(id) DESC) AS actress_rank
FROM top_drama_movies
GROUP BY name
LIMIT 3;
...
SELECT 
    ...
    director_name,
    COUNT(id) AS number_of_movies,
    ROUND(SUM(DATEDIFF(Next_publish_date, date_published)) / (COUNT(id) - 1)) AS avg_inter_movie_days,
